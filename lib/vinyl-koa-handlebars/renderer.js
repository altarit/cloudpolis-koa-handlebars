...
var debug = require("lib/log")(module);
var log = { debug: (message) => { /*debug.trace(message)*/}}; //TODO: add logging
...
function Renderer(options) {
  ...
  if (options) {
    log.debug("creating koa-handlebars instance with options", options);
  } else {
    log.debug("creating koa-handlebars instance");
  }

  var o = this.options = extend({}, Renderer.defaults, options);
  log.debug("using %s as the root", chalk.grey(o.root));

  // custom handlebars instance
  if (o.handlebars) {
    log.debug("using a custom handlebars instance");
...
Renderer.prototype.getFile = function *(file) {
  log.debug("reading file %s", chalk.grey(path.relative(this.options.root, file)));
  return fm(yield fs.readFile(file, "utf8"));
};

Renderer.prototype.compileTemplate = function *(file) {
  var meta = yield this.getFile(file);
  log.debug("compiling template %s", chalk.grey(path.relative(this.options.root, file)));
...
Renderer.prototype.getTemplate = function *(file) {
  ...
  if (o.cache) {
    if (this.cache.peek(key)) { // check for existence
      log.debug("template %s found in cache", chalk.grey(rel));
      return this.cache.get(key);
    } else {
      var template = yield this.compileTemplate(abs);
      log.debug("saving template %s to cache", chalk.grey(rel));
...
Renderer.prototype.findPartials = function *() {
  ...
  log.debug("searching for partials in %s", chalk.grey(dir));
  ...
  log.debug("%s partials found", chalk.green(files.length));
  return files;
};
...
Renderer.prototype.partial = function (name, fn) {
  log.debug("registering global partial %s", chalk.underline(name));
...
Renderer.prototype.helper = function (name, fn) {
  log.debug("registering global helper %s", chalk.underline(name));
...
Renderer.prototype.render = function *(template, locals, options) {
  ...
  log.debug("rendering %s template", chalk.underline(template));

  // extract layout id
  var layoutId = typeof locals.layout !== 'undefined' ? locals.layout : o.defaultLayout;
  ...
  if (layout) {
    log.debug("rendering with layout %s", chalk.underline(layoutId));
...
Renderer.prototype.setJSON = function *(template, locals, options) {
  var o = this.options;

  locals = extend(true, {}, locals);
  options = extend(true, {}, { data: o.data }, options);
  log.debug("sending %s template", chalk.underline(template));
  ...
  //console.log(view);
  var resObj = {};
  resObj.template = view.body;
  resObj.locals = locals;
  //resObj.partials = options;
  //resObj.html = body || view.render(locals, options);
...
Renderer.prototype.middleware = function () {
  ...
  return function *(next) {
    ...
    this.renderView = function *(view, locals, options) {
      ...
      try {
        return l.isAjax
          ? yield self.setJSON(view, l, o)
          : yield self.render(view, l, o);
      } catch (err) {
        log.debug("unable to render view %s", chalk.underline(view), err.stack);
    ...
    this.render = function *(view, locals, options) {
      this.type = this.locals.isAjax ? "json" : "html";
